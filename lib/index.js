...
function createOptions(opts_, defaults) {
    var opts = opts_;
    if (opts) {
...
Batterie.prototype = {
    ...
    serial: function (name, runner, expects) {
        return this.it(name, runner, createOptions(expects, {
            async: true,
            serial: true
        }));
    },
    resolve: function (name, fn, expects) {
        var options = createOptions(expects, {
            expects: 0,
            async: true,
            serial: false
        });
        options.expects += 1;
        return this.it(name, function (t) {
    ...
    it: function it(testName, runner, options_) {
        ...
        var it = new It(batterie, nameStack, runner, opts);
