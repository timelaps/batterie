...
function It(batterie, nameStack, runner, options_) {
    ...
    it.autoExpect = runner ? options.autoExpect : 0;
...
function checkCounters(batterie, it) {
    ...
    if (it && it.expects && !it.skip) {
        ...
        totalExpectations = it.expects + it.autoExpect;
        actualexpectations = expectations.length;
        if (totalExpectations !== expectations.length) {
            msd = {
                it: it,
                pretty: function () {
                    return 'MISSING: expected ' + it.name.join(' ') + ' to have ' + it.expects + ' expectations instead of ' + (actualexpectations - it.autoExpect);
                }
            };
            it.missed = true;
